...
for idx = 1:length(Nc_vector)
    ...
    for k = 0:(Nc - 1)
        subcarrier_matrix(k + 1, :) = 1 / sqrt(T) * exp(1j * 2 * pi * k * f_delta * t_vector);
    end
    pilot_sk = ones(1, Nc);
    xt = zeros(1, Ns);
    for k = 0:(Nc - 1)
        xt = xt + pilot_sk(k + 1) * subcarrier_matrix(k + 1, :);
    end
    ...
    for snr_i = 1:length(EbN0dBvector)
        ...
        BERtheory(snr_i) = 1/2 - sqrt(EbN0) / (2 * sqrt(EbN0 + 1));
    end
    semilogy(EbN0dBvector, BER(idx, :), colors{idx}, 'DisplayName', ['Nc = ' num2str(Nc)])
end
semilogy(EbN0dBvector, BERtheory, 'r--', 'DisplayName', 'BER theory (Rayleigh)')
grid on
legend
xlabel('Eb/N0 (dB)')
ylabel('Bit Error Rate')
title('BER of OFDM over Multipath Fading Channel for Different Nc')
